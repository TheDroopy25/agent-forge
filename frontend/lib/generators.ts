...
export function generateOpenClawYAML(state: AgentState): string {
  ...
  if (data.files.length > 0) {
    const fileList = data.files.map((f) => `    - name: "${f.name}"\n      type: "${f.type}"`).join('\n');
    dataLines.push(`  files:\n${fileList}`);
  }
  if (data.urls.length > 0) {
    const urlList = data.urls.map((u) => `    - "${u}"`).join('\n');
    dataLines.push(`  urls:\n${urlList}`);
  }
  if (data.apiConnections.length > 0) {
    const apiList = data.apiConnections.map((a) => `    - key: "${a.key}"\n      value: "${a.value}"`).join('\n');
    dataLines.push(`  api_connections:\n${apiList}`);
  }
  dataLines.push(`  structured: ${data.structured}`);
  ...
  if (channels.restApi.enabled) {
    channelLines.push(
      `  rest_api:\n    enabled: true\n    webhook_url: "${channels.restApi.webhookUrl}"\n    secret: "${channels.restApi.secret}"`
    );
  }
  if (channels.sms.enabled) {
    channelLines.push(
      `  sms:\n    enabled: true\n    account_sid: "${channels.sms.accountSid}"\n    auth_token: "${channels.sms.authToken}"`
    );
  }
  if (channels.cliOnly) {
...
sub_agents:
  agents:
${subAgentsBlock}
  routing_mode: ${subAgents.routingMode}
...
export function generateAGENTS(state: AgentState): string {
  const { identity, tools, memory, data, subAgents, channels, guardrails, llm } = state;
  const agentName = identity.name || 'Unnamed Agent';
  ...
  if (data.files.length > 0) {
    dataItems.push(`### Files\n${data.files.map((f) => `- \`${f.name}\` (${f.type})`).join('\n')}`);
  }
  if (data.urls.length > 0) {
    dataItems.push(`### URLs\n${data.urls.map((u) => `- ${u}`).join('\n')}`);
  }
  if (data.apiConnections.length > 0) {
    dataItems.push(`### API Connections\n${data.apiConnections.map((a) => `- \`${a.key}\``).join('\n')}`);
  }
  dataItems.push(`\n**Structured data mode:** ${data.structured ? 'enabled' : 'disabled'}`);
  const dataSection = dataItems.length > 0 ? dataItems.join('\n\n') : '_No data sources configured._';
  ...
  const subAgentsSection =
    subAgents.agents.length > 0
      ? [
          `**Role:** ${subAgents.role}`,
          `**Routing Mode:** ${subAgents.routingMode}`,
          `**Max Concurrent:** ${subAgents.maxConcurrent}`,
          '',
          '| ID | Name | Purpose | Model |',
          '|----|------|---------|-------|',
          ...subAgents.agents.map((a) => `| ${a.id} | ${a.name} | ${a.purpose} | ${a.model} |`),
        ].join('\n')
      : '_No sub-agents configured. Agent operates standalone._';

  // Channels section
  const channelItems: string[] = [];
  if (channels.discord.enabled) channelItems.push(`- **Discord** — Guild: \`${channels.discord.guildId || 'not set'}\``);
  if (channels.telegram.enabled) channelItems.push(`- **Telegram** — bot configured`);
  if (channels.whatsapp.enabled) channelItems.push(`- **WhatsApp** — enabled`);
  if (channels.restApi.enabled) {
    channelItems.push(`- **REST API** — webhook: \`${channels.restApi.webhookUrl || 'not set'}\``);
  }
  if (channels.sms.enabled) channelItems.push(`- **SMS** (Twilio) — account configured`);
